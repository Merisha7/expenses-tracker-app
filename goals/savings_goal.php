...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($goal_name) || empty($required_amount) || empty($start_date) || empty($due_date)) {
        ...
        if (count($start_parts) === 3 && count($due_parts) === 3) {
            ...
            // Assuming user_id = 1 for now (without authentication)
            $user_id = 1;
            
            $stmt = $conn->prepare("INSERT INTO savings_goals (user_id, goal_name, required_amount, current_savings, total_budget, start_date, due_date) VALUES (?, ?, ?, ?, ?, ?, ?)");
            
            if ($stmt) {
                $stmt->bind_param('isddiss', $user_id, $goal_name, $required_amount, $current_savings, $total_budget, $start_date_db, $due_date_db);
    ...
    <div class="app-shell">
        ...
        <main class="main-content">
            ...
            <div class="form-container">
                ...
                <form action="/FinTrack-1/goals/set_goals.php" method="post">
